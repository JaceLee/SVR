...
format compact;
%%
[typ, desc, fmt] = xlsfinfo('D:\实习\SVM\libsvm-3.23\matlab\桥头站数据_extrapolated(2h).xls');
% for s =3:4;
% for t =0:3;
load model_沙河(2h)_s3t1.mat
    for sheet=1:length(desc)
s=3;
t=1;
% sheet=1;
[read_data,txt,raw] = xlsread('D:\实习\SVM\libsvm-3.23\matlab\桥头站数据_extrapolated(2h).xls',char(desc(sheet)));
...
%     XX_TUR=read_data((lag_time+1):end_time,16);     %浊度
...
%    XX_NHN=read_data((lag_time+1):end_time,5);    %氨氮
...
%    test_x = [XX_WT XX_TUR XX_cond XX_DO XX_MN XX_NHN XX_TN];
    ...
    grid on;
    print(gcf,'-djpeg','-r600',['D:\实习\SVM\libsvm-3.23\matlab\MIC_PCA\桥头\yearly\Figure\',...
        '2h_s',num2str(s),'_t',num2str(t)]);
    end
    %% 导出文件
    SIMY{sheet} = [ptest];
    % simY(Sheet,1) = {SIMY};
    output = ['D:\实习\SVM\libsvm-3.23\matlab\MIC_PCA\20120415-20130414\yearly\',...
